    ...
    <section className="rounded-3xl border border-neutral-200 p-4 shadow-sm fantasy-card">
      ...
      <div className="overflow-x-auto rounded-2xl shadow-sm border border-neutral-200">
        <table className="table-fixed w-full text-sm 2xl:text-base finite-table">
          ...
          <thead>
            <tr>
              <th className="text-left px-2 py-1 font-semibold">Pronoun</th>
              {TENSES.map(t => (<th key={t.key} className="text-left px-3 py-1.5 font-semibold capitalize text-neutral-900">{t.label}</th>))}
            </tr>
          </thead>
          <tbody>
            {rows.map(({ p, items }, i) => (
              <tr key={p.form} className={i % 2 ? "row-odd" : "row-even"}>
                ...
                {items.map((f, j) => (<td key={j} className="px-3 py-1.5 font-medium">{f}</td>))}
              </tr>
            ))}
          </tbody>
        </table>
